...
try:
    from razdel import sentenize as _sentenize
except ImportError:
    sys.exit("Нужен razdel:  pip install razdel")

# Сокращения, после которых точка предложение не кончает никогда: за ними
# всегда что-то следует. Отдельно «г.», «в.», «т. д.» сюда не входят — ими
# фраза вполне может кончаться («Август 1917 г.»), и склеивать их опасно.
NEVER_FINAL = re.compile(
    r"(?:^|[\s(])(?:гг|тт|стр|см|ср|изд|ред|им|напр|акад|проф|доц|"
    r"тыс|млн|млрд)\.\s*$")

# Сокращение перед закрывающей скобкой: «(1872 г.)» — почти наверняка
# середина фразы, а не её конец.
ABBR_PAREN = re.compile(
    r"(?:^|[\s(])(?:г|гг|в|вв|стр|с|т|тт|ч|н|э|им|см|ср|др|пр|проч|изд|"
    r"ред|обл|руб|коп)\.\)\s*$")


def depth(t):
    """Сколько скобок осталось незакрытыми."""
    return t.count("(") - t.count(")") + t.count("[") - t.count("]")


def sentenize(text):
    """
    Деление на предложения с починкой ложных обрывов.

    Два правила, и первое важнее.

    **Предложение не кончается внутри скобок.** razdel рвёт по точке с запятой
    перед заглавной буквой, и длинное перечисление в скобках разлеталось на
    куски: «(Плеханов… в России;» / «Шейдеман… в Германии;» — хотя это одна
    фраза. Правило общее и не требует никаких списков.

    **Не кончается на сокращении, которым кончиться нельзя** — «гг.», «стр.»,
    «см.» — и на сокращении перед закрывающей скобкой: «(1872 г.)».

    Широкое правило «любое сокращение склеивать» пробовалось и отвергнуто:
    оно слепляло «Август 1917 г.» со следующей фразой.

    Скобочное правило не применяется, если скобки не сходятся во всём абзаце:
    в побитых распознаванием исходниках одна потерянная скобка склеила бы
    абзац целиком.
    """
    parts = list(_sentenize(text))
    use_brackets = depth(text) == 0
    ...
    for s in parts:
        ...
        if glue:
            a = out.pop()
            out.append(_Span(a.start, s.stop, text[a.start:s.stop]))
        else:
            out.append(s)
    return out
...
class _Span:
    ...
    def __init__(self, start, stop, text):
        self.start, self.stop, self.text = start, stop, text
