...
function drawRaw(ranges, live){
  ...
  for(var k = 0; k < ranges.length; k++){
    ...
    for(var m = 0; m < rr.length; m++){ rr[m].__soft = soft; rects.push(rr[m]); }
  }
  ...
  for(var i = 0; i < rects.length; i++){
    ...
    d.className = r.__soft ? 'rawsel soft' : 'rawsel';
...
function anchors(){
  ...
  var fr = allFrags();
  if(fr.length){
    /* цитата, собранная из кусков: охват — по самим кускам, а не по тому,
       что сейчас выделено */
    var sp = spanOf(fr);
    if(sp.length){
      var a = anchorFor(sp);
...
function fragParam(){
  var fr = allFrags();
  if(fr.length < 1 || !cur.length) return '';
  var out = [];
  for(var i = 0; i < fr.length; i++){
    var a = offsetIn(cur, fr[i].startContainer, fr[i].startOffset);
    var b = offsetIn(cur, fr[i].endContainer, fr[i].endOffset);
    if(a >= 0 && b > a) out.push(a + '-' + b);
  }
  /* весь якорь целиком — подсвечивать нечего */
  if(out.length === 1){
    var p = out[0].split('-');
    var total = cur.map(function(e){
      return textNodes(e).map(function(n){ return n.nodeValue.length; })
             .reduce(function(x, y){ return x + y; }, 0);
    }).reduce(function(x, y){ return x + y + 1; }, -1);
    if(+p[0] === 0 && +p[1] >= total) return '';
...
function drawAcc(){
  ...
  if(!bag.length && !fr.length){ markBag(); return; }
...
function addToBag(){
  ...
  if(!bag.some(function(x){ return x.a === a; })){
    ...
    kept = rest;
    bag.push({a: a, f: offsetsFor(cur, sortFrags(here.concat(frags)))});
    save(KACC, bag);
  }
  drawAcc();
...
acc.addEventListener('click', function(e){
  ...
  if(rm){ bag.splice(+rm.dataset.rm, 1); save(KACC, bag); peek([]); drawAcc(); return; }
  if(e.target.closest('[data-clear]')){
    bag = []; save(KACC, bag); peek([]); drawAcc(); return;
  }
  var rf = e.target.closest('[data-rmf]');
  if(rf){
    kept.splice(+rf.dataset.rmf, 1);
    peek([]); clearPeekRaw(); drawAcc();
    if(cur.length) setCur(cur, null, true);
    return;
  }
